//! Companies CRUD HTTP surface (migration `0030_companies.sql`).
//!
//! **Router-registry pattern.** [`router_for`] returns this module's sub-router;
//! [`crate::http::router`] merges it under the bearer auth layer beside
//! `agents::router_for` with one `.merge(...)` line. Mounting under `/api/` is
//! enough — `auth_middleware` has no path carve-outs.
//!
//! **Scope.** Four routes wrap the `db::companies` surface:
...
//!
//! **Slug soft-reject (§9).** A company slug lives in its own namespace and can
//! never collide with a `sessions.name` at the PK level, but for folder/URL
//! legibility the create handler additionally rejects (409) a slug equal to an
//! existing session slug.
//!
//! **Delete guard (§9).** `DELETE` refuses while the company has any active
//! (non-archived) session; the `trg_company_delete_*` triggers then NULL any
//! lingering (archived) sessions and drop connectors, but the `root_dir` folder
//! on disk is NOT removed — the response returns the retained path.
...
/// on disk (§9) — hand the owner the path so they can remove it deliberately.
...
async fn create_handler(
    ...
    // root (§4.1), not a cwd-relative one.
    ...
    // URL legibility, §9) — same 409 shape as a duplicate company slug.
...
async fn patch_handler(
    ...
    // UNIQUE row survived archival, so it cannot have been re-created (§9).
...
async fn delete_handler(
    ...
    // The triggers NULL any lingering (archived) sessions and drop connectors.
    // The `root_dir` folder on disk is deliberately NOT removed (data safety,
    // §9) — hand back the retained path.
