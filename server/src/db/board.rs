...
/// (R1/R2: `WHERE session = <name> AND status = 'doing'`). This is how the
/// session→board reaction resolves "the issue linked to this session". If a
/// session somehow owns more than one `doing` issue, the most-recently-updated
/// one wins (a single deterministic target for the side-effect).
