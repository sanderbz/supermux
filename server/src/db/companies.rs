//! Company row access (migration `0030_companies.sql`).
//!
...
//! there is no `rename`/`set_slug`, decision #2/#3), a mutable `display_name`, a
//! `root_dir` folder every company agent lives under, an `archived` flag, and
//! timestamps. `sessions.company_id` is a plain nullable filter attribute
//! pointing here; `company_id IS NULL` is a main/PA/tech-admin bot.
...
/// `trg_company_delete_*` triggers (0030) then drop its `company_connectors`
/// and NULL any lingering sessions' `company_id`. The `root_dir` folder on disk
/// is NOT removed — that is the caller's deliberate act (§9).
...
/// it (a company with live members cannot be hard-deleted, §9).
