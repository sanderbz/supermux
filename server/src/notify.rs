...
impl PushPayload {
    ...
    pub fn simple(title: impl Into<String>, body: impl Into<String>, url: impl Into<String>, tier: Tier) -> Self {
        let url = url.into();
        Self {
            title: title.into(),
            body: body.into(),
...
fn compose_body(ev: &NotifEvent, ctx: &ComposeCtx) -> String {
    match ev {
        ...
        NotifEvent::AgentNotice(msg) => {
            one_line_capped(msg, PUSH_BODY_MAX).unwrap_or_else(|| "The agent needs your input.".to_string())
        }
        ...
        NotifEvent::TurnFailed { etype, msg } => {
            ...
            one_line_capped(&joined, PUSH_BODY_MAX).unwrap_or_else(|| "The turn failed.".to_string())
        }
        ...
        NotifEvent::SessionCrashed { .. } => match ctx.activity_label.as_deref().map(str::trim) {
            Some(label) if !label.is_empty() => {
                cap(&format!("{} — {label}", FALLBACK_CRASHED.trim_end_matches('.')))
            }
...
pub async fn deliver(state: &AppState, session: &str, ev: NotifEvent) {
    ...
    if matches!(ev.tier(), Tier::Unread) && state.chat_store(session).is_some() {
        tracing::debug!(session, "notify: unread suppressed — the session is being viewed");
        return;
    }
...
pub async fn build_payload(state: &AppState, session: &str, ev: &NotifEvent) -> Option<PushPayload> {
    let row = db::sessions::get(&state.pool, session).await.ok().flatten()?;
...
/// permission dialog, an un-answered agent notice, or an un-cleared error.
///
...
pub fn attention_badge(state: &AppState) -> u32 {
    state
        ...
        .filter(|e| {
            let a = e.value();
            a.permission.is_some() || a.notice.is_some() || a.error.is_some()
        })
        .count()
...
pub async fn tail_for_push(state: &AppState, row: &db::sessions::Session) -> Option<ChatTail> {
    ...
    let agent = tokio::task::spawn_blocking(move || last_assistant_line(&path))
        ...
        .ok()
        .flatten()?;
    Some(ChatTail { user: String::new(), agent, ts: 0 })
}
...
mod tests {
    ...
    fn exit_plan_mode_is_a_plan_approval_not_a_permission_sentence() {
        ...
        assert_eq!(p.url, "/focus/deploy-fix#pending", "tap lands on the plan card");
    }
    ...
    fn a_finished_turn_carries_the_agents_actual_closing_line() {
        ...
        assert_eq!(p.url, "/focus/deploy-fix", "a calm tier does not deep-link to a dialog");
    ...
    fn a_long_closing_line_is_capped_at_the_banner_budget_on_a_word_boundary() {
        ...
        c.tail = Some(ChatTail { user: String::new(), agent: long, ts: 1 });
        let body = compose(&NotifEvent::TurnFinished, &c).body;
        assert!(body.chars().count() <= PUSH_BODY_MAX, "{} chars", body.chars().count());
        assert!(body.ends_with("alpha"), "cut on a word boundary: {body:?}");

        // Multi-byte safety: 500 scalars of `é` with no whitespace at all.
        c.tail = Some(ChatTail { user: String::new(), agent: "é".repeat(500), ts: 1 });
        let body = compose(&NotifEvent::TurnFinished, &c).body;
    ...
    fn a_finish_with_no_recoverable_tail_falls_back_to_the_declared_sentence() {
        assert_eq!(compose(&NotifEvent::TurnFinished, &ctx()).body, "Turn finished.");
        // An empty agent line is "no tail", not an empty push.
        let mut c = ctx();
        c.tail = Some(ChatTail { user: "hi".to_string(), agent: String::new(), ts: 1 });
        assert_eq!(compose(&NotifEvent::TurnFinished, &c).body, "Turn finished.");
    }
    ...
    fn a_failed_turn_without_a_message_does_not_trail_a_colon() {
        let p = compose(
            &NotifEvent::TurnFailed { etype: "rate_limit".to_string(), msg: String::new() },
            &ctx(),
        );
    ...
    fn a_crash_names_the_last_thing_it_was_doing_when_there_is_one() {
        ...
        let p = compose(&NotifEvent::SessionCrashed { reason: "other".to_string() }, &c);
        assert_eq!(p.body, "Session ended unexpectedly — ⚡ cargo check");
        assert_eq!(p.tier, Tier::Error);

        c.activity_label = None;
        assert_eq!(
            compose(&NotifEvent::SessionCrashed { reason: String::new() }, &c).body,
    ...
    fn the_title_is_the_bots_name_never_the_slug_and_never_a_sentence() {
        ...
        assert!(!p.title.starts_with("agent "), "no `agent {{name}}` prefix survives");
    ...
    fn every_session_event_coalesces_into_one_slot_per_session() {
        ...
        for ev in [
            ...
            NotifEvent::TurnFailed { etype: "e".into(), msg: "m".into() },
            NotifEvent::SessionCrashed { reason: "other".into() },
        ] {
            let p = compose(&ev, &c);
            assert_eq!(p.tag, "session:deploy-fix", "{ev:?} shares the session slot");
    ...
    fn tiers_and_categories_are_pinned_per_event() {
        use NotifEvent::*;
        for (ev, tier, cat) in [
            (PermissionAsked, Tier::Attention, NotifCategory::AgentWaiting),
            (AgentNotice("x".into()), Tier::Attention, NotifCategory::AgentWaiting),
            (Question("x".into()), Tier::Attention, NotifCategory::AgentWaiting),
            (TurnFinished, Tier::Unread, NotifCategory::AgentFinished),
            (
                TurnFailed { etype: "e".into(), msg: "m".into() },
                Tier::Error,
                NotifCategory::AgentError,
            ),
            (
                SessionCrashed { reason: "other".into() },
                Tier::Error,
    ...
    fn the_schedule_lane_keeps_its_own_url_tag_and_has_no_session_slot() {
        ...
        assert_eq!(p.session, None, "no session policy applies to the schedule lane");
        assert!(p.renotify);
    }
    ...
    fn last_assistant_line_reads_the_newest_main_agent_text_off_disk() {
        ...
        let dir = std::env::temp_dir().join(format!("supermux-notif-tail-{}", uuid::Uuid::new_v4()));
