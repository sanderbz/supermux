...
**Secret-floor interplay.** The per-company `CLAUDE_CONFIG_DIR` secret-floor (§6) is set on the **child env per-spawn** and is a floor that holds on **every** host and backend, including Noop — so even a mechanism-less host relocates each company's connector OAuth/MCP tokens out of a co-resident `~/.claude.json`. The Landlock allow-list grants that per-company config dir RW and denies the others, so on a Full host the floor is additionally kernel-backed.
...
**The secret-floor: per-company `CLAUDE_CONFIG_DIR` (P0, every host, every backend).** Materialization must **not** write decrypted secrets into a single shared `~/.claude.json`. Instead, every company agent is spawned with a **per-company `CLAUDE_CONFIG_DIR`** set on the **child env per-spawn** (`mkdir 0700`), so Claude's `~/.claude.json` — the connector OAuth/MCP tokens — is relocated per company and is **never co-resident** with another company's. Company-A's materialized Slack token lands in Company-A's config dir; Company-B's shell has a *different* `CLAUDE_CONFIG_DIR` and no path to it (kernel-denied on a Landlock/Seatbelt host per §4.4; env-separated even on Noop). **Critical implementation note:** this MUST be set on the **child env per-spawn**, not process-global. The server currently treats `CLAUDE_CONFIG_DIR` process-globally (which is exactly why the `chat_ws` harness serializes env tests on `ENV_LOCK`); a process-global value would defeat isolation. Per-child is what makes it isolate. This is the **secret-floor**, and it holds even on a mechanism-less host — it is P0, co-shipped with the data model, not a deferred follow-on.
...
**P0 — invisible data-model foundation + the secret-floor.** Migration 0030 (companies, `sessions.company_id`, seeded `human_users` owner, `company_connectors` table + triggers). Thread `company_id` through the five Rust types + web types (§3.3). Bump the `db/mod.rs:116` assertion to 28. Companies CRUD endpoints + `db/companies.rs`. **Create-time dir forcing ships here, co-shipped with `company_id`-on-create (§4.1):** the column and its dir invariant must land together — a company session created in P0 whose `dir` merely defaulted to `$HOME` would be un-jailable when P1 flips the jail, since P1 does not retroactively move it. The same forcing covers the `duplicate` seam (§3.3#3). **The secret-floor (§6, §4.4) also ships here:** every company session is spawned with a per-company `CLAUDE_CONFIG_DIR` (`mkdir 0700`) set on the **child env per-spawn** (never process-global), and the connector store's materialization (P2) targets that dir — the floor holds on every host, including a mechanism-less one, and is the P0-critical isolation primitive. **Done when:** `cargo check` (debug) + `bun run build:perf` + `tsc -b` are green, the new migration count test passes, a curl round-trip creates a company and a company-scoped session whose `dir` is forced under `root_dir`, and two concurrently-spawned company sessions resolve to *different* `CLAUDE_CONFIG_DIR` values (a per-child-env test, not process-global). **Owner sees:** nothing visually changes in the dashboard (that is the point); the owner verifies via a create/list curl that a company exists and a session carries its `company_id`.
