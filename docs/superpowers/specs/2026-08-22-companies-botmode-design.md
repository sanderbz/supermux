...
**Goals, in priority order.** (1) A server-side isolation boundary — filesystem (the Files browser/API), agent-to-agent delegation, and connector-credential *fetching* — enforced in the server, not merely in the client UI. **Scope of goal (1) (honest limit, updated for the portable-isolation decision):** the agent's *own shell* is now confined by a **kernel-enforced OS sandbox** — the portable `IsolationProvider` (Landlock on Linux, Seatbelt on macOS), applied per-spawn to company agents (§4.4). On a capable host (Linux + the `@sandbox` unit line) a company agent's shell **cannot** `cat` a sibling company's folder, its per-company `CLAUDE_CONFIG_DIR`, or `~/.supermux/auth_token`; the filesystem jail (§4.1) additionally confines the Files browser/API, and delegation + the connector-manager query remain server-enforced. Only on a **mechanism-less host** does this degrade to the **secret-floor** (per-company `CLAUDE_CONFIG_DIR`, every host) + a deliberate **co-tenant trust** assumption — agents are the owner's own bots, and `BestEffort` mode logs one loud warning rather than crashing. The P3 boundary that must hold — the *human's* reach — is fully server-enforced independent of the OS sandbox. (This supersedes the earlier draft's flat "every agent shell is unconfined" framing; per-process users stay a non-goal, but OS sandboxing is now in — §12.) (2) KISS: no docker, no namespaces, no per-process users, no per-company server instance, no re-keying of the ~20 slug-keyed in-memory maps. (3) Incremental delivery: four phases, each independently shippable and visually verifiable by the owner. (4) Zero regression for the existing fleet: today's company-less bots keep behaving exactly as they do now.
...
Four mechanisms. Three of them (§4.1 filesystem jail, §4.2 delegation gate, §4.3 query scoping) are server-enforced API/UI boundaries that are cheap because the seam already exists. The fourth (§4.4 portable agent isolation) is the **new primary boundary for the agent's own shell** — a kernel-enforced OS sandbox (Landlock on Linux, Seatbelt on macOS) applied per-spawn to company agents, with an honest degrade to secret-floor + co-tenant trust only on a mechanism-less host. On a capable host the agent shell can no longer `cat` a sibling company's tree or the shared secrets; the co-tenant-trust framing that the earlier draft of this spec assumed is now the *fallback*, not the primary path.
...
### 4.4 Portable agent isolation (owner-approved — the primary agent-shell boundary)

§4.1–§4.3 confine the *owner/human UI surfaces* (Files browser/API, delegation, query results). They do **not** confine the agent's own interactive shell — which can otherwise `cat` any absolute path, including a sibling company's tree and the shared secrets. This subsection closes that gap with an **OS-level sandbox applied per-spawn**, so the agent-shell boundary is kernel-enforced on a capable host and degrades honestly elsewhere. OS sandboxing via Landlock/Seatbelt is therefore now **in scope**, no longer a non-goal (§12).

**The trait (thin, in-tree — modeled on the Codex CLI backend split).** A small `IsolationProvider` abstraction lives in-tree; we do **not** depend on the `birdcage` crate (archived Jul 2026 + GPL-3.0 — it would relicense-contaminate an OSS project). Shape:

```rust
enum IsolationMode { Off, BestEffort, StrictRequired }      // requested policy

enum IsolationLevel {                                        // MEASURED result
    None,                                                    // nothing enforced
    Partial { backend: Backend, note: String },              // e.g. old-kernel, macOS v1
    Full { backend: Backend },                               // fully enforced
}

trait IsolationProvider {
    /// Applied INSIDE the spawned child, post-fork / pre-exec, so it is
    /// inherited across exec and is unescapable.
    fn confine(&self, spec: &ConfineSpec) -> io::Result<()>;
    /// Fork a throwaway child that ACTUALLY restricts itself and reports the
    /// enforced level via exit code (the startup probe).
    fn probe(&self) -> IsolationLevel;
}
// Backends: LandlockLinux, SeatbeltMacOS (cfg(target_os="macos")), Noop.
```

**Gating (critical — mirrors the rest of the spec's `company_id IS NULL` lever).** Confinement is applied **per-spawn ONLY when `company_id != NULL`**. Main/PA/tech-admin bots (`company_id IS NULL`) are never confined — `confine()` is simply not called for them, so there is no global sandbox to carve exemptions out of. This is the same one-fact gate that defines a main bot everywhere else in this spec.

**`IsolationMode` (the requested policy).**
- **`Off`** — escape hatch; `confine()` never called.
- **`BestEffort`** (default) — apply the strongest backend available on this host; if none is available, log **one loud per-session warning** and continue (never crash on a mechanism-less host — the right OSS default for a semi-trusted, model-driven prompt-injection / confused-deputy threat model). BestEffort must surface the **measured** `IsolationLevel`, never the requested mode — it never over-claims.
- **`StrictRequired`** — opt-in fail-closed: **refuse to start** a company session if isolation is unavailable or below a configured ABI floor.

**`IsolationLevel` (the measured result).** Derived from `RulesetStatus::{FullyEnforced|PartiallyEnforced|NotEnforced}` on Linux, or seatbelt profile-compile success on macOS, and surfaced **per session** in logs and (P2) the session UI badge. BestEffort surfaces this, not the mode.

**LandlockLinux backend.** Uses the `landlock` (rust-landlock) crate at `ABI::V4` + `CompatLevel::BestEffort`. The `Ruleset` is built and `restrict_self()` is called **inside** the spawned child via `std::os::unix::process::CommandExt::pre_exec` (post-fork / pre-exec), so the restriction is inherited across `exec` and is unescapable by the agent. Allow-list: the company tree (RW) + the company's own `CLAUDE_CONFIG_DIR` (RW) + the RO/exec system paths a dev shell needs (`/usr`, `/lib`, `/lib64`, `/bin`, `/etc`, the node/cargo/rustup caches, `/tmp` RW). Deny everything else — including sibling company trees and `~/.supermux/auth_token`. An optional ABI-v4 TCP-port rule (P2) denies the loopback-API port.

**SeatbeltMacOS backend (P3 / on-demand — deferred until the Mac mini joins).** Shells out to a hardcoded `/usr/bin/sandbox-exec -p <profile> -D COMPANY_DIR=… -D SECRETS=… -- <cmd>` as a plain `std::process::Command` — **no** FFI/`libsandbox` link, no entitlements; the profile is kernel-inherited across the child's `exec`. v1 profile = KISS allow-default + deny-cross-company-write + deny-read-secrets ⇒ reports **`Partial`** (explicitly **not** a full read-jail). Escape hatch (`excludedCommands` / `dangerouslyDisableSandbox`) for the three known breakage classes: Go-CLI TLS (`gh`/`gcloud`/`terraform`), Apple Events `-600`, nested Playwright/Chromium. Until the Mac mini joins, macOS runs **Noop + the secret-floor honestly** (never claims a jail it does not have).

**Startup probe (once, not per-spawn).** Fork a throwaway child that **actually** calls `restrict_self()` with the intended attributes and reports `RulesetStatus` via exit code. This is the only way to catch the systemd `@system-service` syscall filter silently blocking `landlock_*` (`EPERM`/`ENOSYS`) on an otherwise Landlock-capable kernel — a runtime feature check alone would lie. It doubles as a deploy self-test: it flips `None → Full` the instant the `@sandbox` unit line (below) lands, with no code change. The macOS probe = a trivial no-op `sandbox-exec` profile-compile.

**Deploy prereq — the `@sandbox` systemd unit line (owner-approved, independently shippable).** The `landlock_*` syscalls live in the systemd `@sandbox` set, which the service's current `SystemCallFilter=@system-service` **blocks**. Add `SystemCallFilter=@sandbox` to `supermux.service` via the root deploy path-unit. It does **not** open namespaces (`RestrictNamespaces` stays `true`) and grants no runtime privilege. Ship it early and independently; the startup probe verifies it live and flips the measured level the moment it takes effect.

**Secret-floor interplay.** The per-company `CLAUDE_CONFIG_DIR` secret-floor (§6) needs **both** halves — a distinct per-company `CLAUDE_CONFIG_DIR` on the **child env per-spawn** **and** company-aware server-side resolvers (the server otherwise resolves Claude's config dir from its own process env: `claude_config_dir`/`claude_json_path` at `server/src/claude_tools/atomic.rs:20-46`, plus `sessions/resumable.rs:70`, `sessions/recall.rs`, `sessions/chat/statusline.rs`, `claude_config.rs:150`, `teams/scan.rs`, `skills.rs`). It lands in **P2**, co-shipped with connector materialization (not P0), because the child-env half alone would leave the server reading/writing `~/.claude(.json)` for a company session. Once both halves are in, even a mechanism-less host relocates each company's connector OAuth/MCP tokens out of a co-resident `~/.claude.json`. The Landlock allow-list (P1) grants that per-company config dir RW and denies the others, so on a Full host the floor is additionally kernel-backed. Until P2 lands both halves, company children share the server's config dir — acceptable because P0/P1 are single-owner (the owner trusts their own bots; no external humans until P3).

**Per-host behavior MATRIX (what `BestEffort` actually reports).**

| Host / condition | Backend | Measured `IsolationLevel` | Behavior |
| --- | --- | --- | --- |
| Linux, kernel ≥ 5.13 (ABI v4), `@sandbox` present | LandlockLinux | **Full** { landlock } | company tree RW, everything else denied; kernel-enforced |
| Linux, Landlock-capable kernel, **no** `@sandbox` (filter blocks `landlock_*`) | LandlockLinux → fails | **None** + **one loud per-session warning** | shell unconfined; secret-floor still applied; probe says why |
| Linux, older kernel (partial ABI, e.g. no v4 net rule) | LandlockLinux | **Partial** { landlock, note } | FS rules enforced (`PartiallyEnforced`); missing rules degraded via `CompatLevel::BestEffort` |
| macOS (Mac mini, once joined) | SeatbeltMacOS | **Partial** { seatbelt, note } | deny-cross-company-write + deny-read-secrets; NOT a full read-jail (v1) |
| macOS before the backend ships / mechanism-less / unknown OS | Noop | **None** | secret-floor only; loud warning; honest — no jail claimed |

Under `StrictRequired`, any row that measures below the configured floor **refuses to start** the company session instead of degrading.

**Honest residuals on kernel 6.8 (documented, not hidden — see §12 Risks).** No abstract-socket / signal IPC scoping between two confined same-UID agents (needs ABI v6 / kernel 6.12; a companion seccomp deny-filter can partially close it — optional/later); TCP-port-only egress (no per-host/IP, no UDP); no defense against kernel 0-days or microarchitectural side channels. Accepted threat model = semi-trusted, model-driven prompt-injection / confused-deputy.
...
**The secret-floor: per-company `CLAUDE_CONFIG_DIR` (P2 — needs BOTH halves, every host, every backend).** Materialization must **not** write decrypted secrets into a single shared `~/.claude.json`. The floor is **two halves that must co-ship**: (a) every company agent is spawned with a **per-company `CLAUDE_CONFIG_DIR`** set on the **child env per-spawn** (`mkdir 0700`), so the child Claude's `~/.claude.json` — the connector OAuth/MCP tokens — is relocated per company; **and** (b) the **server-side resolvers are made company-aware** — the server otherwise resolves Claude's config/json/transcript/projects dir from its **own** process env (`claude_config_dir`/`claude_json_path` at `server/src/claude_tools/atomic.rs:20-46`, `sessions/resumable.rs:70`, `sessions/recall.rs`, `sessions/chat/statusline.rs`, `claude_config.rs:150`, `teams/scan.rs`, `skills.rs`), so with only half (a) set the server keeps reading `~/.claude(.json)` for the company session (breaking `--resume` detection, transcript recall, and statusline) and `write_json_atomic` materialization writes the **server's** `~/.claude.json`, defeating the floor at the write end. With both halves, Company-A's materialized Slack token lands in Company-A's config dir and Company-B's shell has a *different* `CLAUDE_CONFIG_DIR` and no path to it (kernel-denied on a Landlock/Seatbelt host per §4.4; env-separated even on Noop). **Critical implementation note (the LOW correction):** half (a) MUST be on the **child env per-spawn**, not process-global — the server resolves `CLAUDE_CONFIG_DIR` by **reading** `std::env::var` in its resolvers; it does **not** `set_var` it in production (the `chat_ws` harness serializes env tests on `ENV_LOCK` only because the **tests** set the process-global, not because production does). A company session therefore needs a distinct per-company `CLAUDE_CONFIG_DIR` on the child **and** the server-side resolvers must resolve that same per-company dir. This is the **secret-floor**; it lands in **P2, co-shipped with connector materialization** (not P0). Until then, company children share the server's config dir (single-owner trust, §4.4).

**Reconciling the secret-floor with the vault.** Two distinct guarantees at two distinct layers:
- The **vault** (below) protects the **at-rest-in-DB** copy of `config_json` and the **P3-over-the-API** copy (masked on read) — it stops a DB-file/backup reader and a tunneled human from seeing plaintext.
- The **secret-floor** (per-company `CLAUDE_CONFIG_DIR`) stops **runtime co-residence** — the materialized *plaintext* no longer lands in one shared file, so a sibling company's running agent cannot `cat` it. On a §4.4 Full host this is additionally kernel-enforced (the Landlock allow-list grants only *this* company's config dir); on a mechanism-less host the env separation is the floor and co-tenant trust covers the residual (a determined same-UID shell could still traverse to another dir absent the kernel jail). The two together — not either alone — remove the earlier draft's "a Company-B bot can read Company-A's live secrets" limitation on any capable host, and reduce it to secret-floor + co-tenant trust on a mechanism-less one.

**The encrypt-at-rest vault (P2, new — decision #9).** No vault exists today — secrets are plaintext in `~/.claude.json` (`claude_tools/atomic.rs`), masked only on read (`mask_mcp_secrets`, `atomic.rs:127-142`). Decision #9 locks the vault into P2, so it is built here. **Scope of its guarantee (narrow, honest):** it seals `company_connectors.config_json` **at rest in the DB** and keeps the plaintext from round-tripping to any **P3 human over the API** (the manager sheet masks; see §7). It does **not**, by itself, isolate the plaintext between *running* agents — that is the **secret-floor's** job (the per-company `CLAUDE_CONFIG_DIR` above — child env **plus** company-aware server resolvers — kernel-backed by §4.4 on a capable host), a separate P2 mechanism. The vault and the secret-floor are complementary layers, not substitutes. The at-rest sealing still earns its P2 slot: it removes plaintext credentials from the DB file and from any DB backup/snapshot, and it is the exact primitive §7 depends on when tunneled humans first become DB-adjacent readers. Model the key handling on the existing `auth_token`-file flow (`server/src/config.rs:309-367`): a `<data_dir>/companies_vault_key` file, mode `0600`, generated on first use (32 random bytes), overridable by `SUPERMUX_COMPANIES_VAULT_KEY` env. `config_json` is sealed with an AEAD (XChaCha20-Poly1305 / libsodium-style secretbox) under that key, stored as `{nonce, ciphertext}` base64. Decryption happens only server-side at materialization; the connector-manager sheet keeps the existing masking (`"••• set"`) so values never round-trip to any client. Key rotation re-seals every row under a new key in one pass.
...
**Deploy prereq (independently shippable, before or alongside P0) — the `@sandbox` unit line.** Add `SystemCallFilter=@sandbox` to `supermux.service` via the root deploy path-unit (§4.4). It opens no namespaces and grants no runtime privilege; it merely un-blocks the `landlock_*` syscalls the current `@system-service` filter denies. Ship it early — it is self-verifying (the P1 startup probe flips `None → Full` the instant it takes effect) and carries no behavior change until the LandlockLinux backend lands.

**P0 — invisible data-model foundation (data model + companies CRUD + dir-forcing, behavior-neutral for NULL sessions).** Migration 0030 (companies, `sessions.company_id`, seeded `human_users` owner, `company_connectors` table + triggers). Thread `company_id` through the five Rust types + web types (§3.3). Bump the `db/mod.rs:116` assertion to 28. Companies CRUD endpoints + `db/companies.rs`. **Create-time dir forcing ships here, co-shipped with `company_id`-on-create (§4.1):** the column and its dir invariant must land together — a company session created in P0 whose `dir` merely defaulted to `$HOME` would be un-jailable when P1 flips the jail, since P1 does not retroactively move it. The same forcing covers the `duplicate` seam (§3.3#3). **The secret-floor does NOT ship in P0 — it moved to P2** (see below and §6/§4.4): it needs both the child-env `CLAUDE_CONFIG_DIR` **and** company-aware server resolvers, so it co-ships with connector materialization. P0 stays behavior-neutral for existing (NULL) sessions. **Done when:** `cargo check` (debug) + `bun run build:perf` + `tsc -b` are green, the new migration count test passes, and a curl round-trip creates a company and a company-scoped session whose `dir` is forced under `root_dir`. **Owner sees:** nothing visually changes in the dashboard (that is the point); the owner verifies via a create/list curl that a company exists and a session carries its `company_id`.

**P1 — isolation + switcher (the first visible slice).** Flip the Files jail (`None → company root`, §4.1) — create-time dir forcing already landed in P0, so the jailed roots exist to flip onto; add the delegation gate + silent-404 + graph filtering (§4.2); ship the `<CompanySwitcher>`, `activeCompany` persisted state, whole-app scoping, new-agent default, team-card scoping, and global-search-company-first (§5). **Portable agent isolation lands here (§4.4):** the in-tree `IsolationProvider` trait, the `IsolationMode` enum (`Off`/`BestEffort`-default/`StrictRequired`), the `IsolationLevel` measured-result surfacing in logs, the **startup probe** (fork-and-`restrict_self` self-test — catches the `@system-service` filter silently blocking `landlock_*`), and the **LandlockLinux backend** (rust-landlock `ABI::V4` + `CompatLevel::BestEffort`, `restrict_self()` via `pre_exec` inside the company-agent child; applied per-spawn only when `company_id != NULL`; main/PA bots never confined). `BestEffort` surfaces the measured level and logs one loud warning if no mechanism is available; it never crashes a mechanism-less host. **Done when:** the switcher narrows the overview + teams; a company bot's Files view cannot escape its root; a cross-company delegate curl returns 404; a same-company and a main-bot delegate succeed; the startup probe reports `Full` on the deployed host (with the `@sandbox` line) and a confined company agent's shell **cannot** `cat` a sibling company's tree or `~/.supermux/auth_token` while a main bot still can; gates green. **Owner sees:** picks a company in the switcher → the roster, team cards, and Files all narrow; a new agent lands in that company's folder; the PA still reaches everyone; the session log records the measured isolation level.

**P2 — secret-floor + connector store + vault + isolation surfacing.** **The secret-floor lands here (both halves, §6/§4.4):** a per-company `CLAUDE_CONFIG_DIR` (`mkdir 0700`) on the **child env per-spawn** (never process-global) **AND** company-aware server-side resolvers — parameterize `claude_config_dir()`/`claude_json_path()` (`claude_tools/atomic.rs:20-46`) to take an explicit per-session config dir and thread it through `sessions/resumable.rs:70`, `sessions/recall.rs`, `sessions/chat/statusline.rs`, `claude_config.rs:150`, `teams/scan.rs`, `skills.rs` — so `--resume`, transcript recall, statusline, and `write_json_atomic` all resolve the same per-company dir. `company_connectors` behavior: shared vs per-bot (name-matched wholesale replace), session-start materialization via `write_json_atomic` into that per-company `CLAUDE_CONFIG_DIR`, the encrypt-at-rest vault, and the manager-sheet company scope + inherited/overridden UI (§6). **Isolation hardening also lands here (§4.4):** the per-session `IsolationLevel` **UI badge** (surfacing the measured `Full`/`Partial`/`None` per session, never the requested mode), the **`StrictRequired`** fail-closed mode (refuse to start a company session below the configured ABI floor), and the optional **ABI-v4 TCP-port egress rule** denying the loopback-API port to confined agents. **Done when:** two companies each hold a differently-credentialed Slack; a bot starts and its `local` scope (in its own config dir) shows exactly its company's (and any per-bot-override) connectors; a company session's transcript **recall** + **`--resume`** detection + **statusline** all resolve against the per-company config dir (not the server's `~/.claude`) and `write_json_atomic` materializes into the per-company dir (not the server's `~/.claude.json`) — not merely that two env values differ; secrets are sealed at rest and masked on read; a session shows its measured isolation badge; a `StrictRequired` company session refuses to start on a host that measures below floor; gates green. **Owner sees:** configures Slack for Company X and Company Y with different tokens; each company's bot picks up its own on start; the sheet shows "inherited from company"; each session shows its isolation level.

**P3 (and on-demand) — external humans + the macOS backend.** Per-company Cloudflare tunnel + Origin allowlist; Google login + human-auth middleware beside the bearer; per-company SSE channels; per-message human author provenance through `send_harness_text`; `author_user_id`/`company_id` on `audit_log`; iCal gating; owner/admin/member permissions; the connector-manager per-human isolation (§7). **The `SeatbeltMacOS` backend (§4.4) is on-demand, deferred until the Mac mini joins the fleet:** `sandbox-exec -p <profile>` shelled as a plain `Command` (no FFI/entitlements), v1 profile reporting `Partial` (deny-cross-company-write + deny-read-secrets, not a full read-jail), with the `excludedCommands`/`dangerouslyDisableSandbox` escape hatch for the three known breakage classes. Until it ships, macOS runs Noop + the secret-floor and the badge reports `None` honestly. **Done when:** a Google-authenticated Company-B colleague reaches only Company-B agents (list, files, search, delegate all hard-scoped), each of their messages renders "●name" in the transcript, the audit ledger attributes actions to them, and no Company-A SSE/secret/iCal data reaches them; gates green. **Owner sees:** invites a colleague to one company; that colleague logs in via Google, chats with only that company's agents, and the owner sees the colleague's name on each message and in the audit log.
...
- **Runtime secret co-residence — now closed on a capable host, degrades honestly (§4.4, §6).** The **primary** path is now kernel-enforced: the per-company `CLAUDE_CONFIG_DIR` secret-floor (P2 — child-env per-spawn **plus** company-aware server resolvers, §6/§4.4) relocates each company's materialized secrets, and the LandlockLinux/Seatbelt backend (P1/on-demand) denies a confined agent's shell any path to a sibling company's tree, config dir, or `~/.supermux/auth_token`. On a Linux+`@sandbox` host this measures **Full** and a Company-B bot **cannot** read Company-A's secrets at runtime. On a **mechanism-less host** it degrades to secret-floor (env separation) + **co-tenant trust** — the vault still covers at-rest-in-DB + P3-over-the-API, and the agents are the owner's own bots. The human boundary P3 must enforce is unaffected either way (humans never get a shell). A single global vault key sealing every row remains a documented simplification (a per-company key is a later hardening, not a v1 requirement).
- **Kernel-6.8 isolation residuals (honest, documented — do not hide).** On kernel 6.8 the Landlock backend cannot scope **abstract-socket / signal IPC** between two confined same-UID agents (needs ABI v6 / kernel 6.12; a companion seccomp deny-filter can *partially* close it — optional/later). Egress control is **TCP-port only** (no per-host/IP rule, no UDP). And no OS sandbox defends against **kernel 0-days or microarchitectural side channels**. Accepted threat model = semi-trusted, model-driven prompt-injection / confused-deputy — not a malicious native co-tenant with a kernel exploit.
- **`@sandbox` filter silently blocking Landlock.** On a Landlock-capable kernel, the systemd `@system-service` syscall filter blocks `landlock_*` (`EPERM`/`ENOSYS`) so the sandbox silently fails to `None`. Mitigated by (a) the **deploy prereq** adding `SystemCallFilter=@sandbox`, and (b) the **startup probe** that actually calls `restrict_self()` and reports the real measured level, so a silent failure is surfaced (loud per-session warning), never hidden as a false `Full`.
...
- **Docker / containers and microVMs / gVisor.** Rejected as the agent-isolation mechanism — OOM-hostile on the small hosts (CLAUDE.md: never `--release`; deploy hosts thrash) and the wrong threat model for a semi-trusted, model-driven agent. OS-level isolation is instead the in-tree Landlock/Seatbelt `IsolationProvider` (§4.4), which is now **in scope** (this is the one prior non-goal that was reversed by the owner-approved decision).
- **A per-UID privileged spawn-helper (later hardening milestone, not v1).** A helper that spawns each company agent under a distinct UID is the strongest same-host DAC boundary — the point where POSIX permissions finally bite — but it reintroduces a **privileged runtime component** the current no-caps design deliberately omits. Explicitly a later milestone, not v1. (Its absence is the reason §4.4's residuals on same-UID IPC scoping exist.)
- **Per-company vault key.** One global `companies_vault_key` seals every company's rows in v1; a per-company key is a later hardening, not a v1 requirement.
