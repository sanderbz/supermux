...
/**
 * The server clipped this message at the wire cap. Say so — a clipped message
 * that just stops reads as an answer that ended mid-sentence, and the reader
 * has no way to tell the difference.
 *
 * Carried over from the A1 panel when A3 replaced its render tree: the flag is
 * the SAME one `entries.ts` derives from the wire (`ChatItem.truncated`), and
 * `recall.rs` still sets it — dropping the marker would have made the cap
 * silent again, which is exactly what the fix was about.
 */
function ClippedMarker() {
  return (
    <span
      className="ml-1 select-none align-baseline text-[12px] italic text-ink-2"
