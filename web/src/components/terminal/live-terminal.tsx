...
export function LiveTerminal({
  ...
  return (
    ...
    >
      <div
        ref={containerRef}
        // `touch-action: none`: xterm 6.0 has no built-in touch-drag scroll, so
        // we drive the scrollback from a touch→scrollLines handler in
        // use-live-term. `none` hands us every touch event AND blocks the
        // browser's native gesture (a vertical drag would otherwise pan the page
        // / expand the mobile browser chrome instead of scrolling). See the
        // globals.css `.xterm` touch-action rule.
        //
