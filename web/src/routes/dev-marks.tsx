...
/* ── the Grok skin showcase (the jury board for the expressive faces) ────────── */

/** Every face-state under the skin, in the order a session moves through them. */
const GROK_STATES: readonly MarkState[] = [
  'connecting',
  'idle',
  'working',
  'thinking',
...
/** A one-line "what it says" caption per state — the pre-attentive read. */
const GROK_STATE_NOTE: Readonly<Record<MarkState, string>> = {
  connecting: 'coming online — eyes scan L↔R',
  idle: 'awake, calm, slightly dim',
  working: 'heads-down — narrowed, in-hue glow',
  thinking: 'reasoning — eyes up & aside',
  streaming: 'talking — own-hue glow pulses',
  waiting: 'wide-eyed, micro-glancing',
  done: 'finished cleanly — content squint',
  stopped: 'asleep — held shut',
  failed: 'fell over — knitted brow + shake',
}

const GROK_SIZES = [18, 24, 32, 44] as const

/** A `[data-grok]` island (with the `.dark` ancestor when dark) on the grok page
 *  paper, so the negative-space eyes read as cut-outs and every scoped rule and
 *  keyframe applies exactly as it does in the real shell. */
function GrokIsland({
  theme,
  children,
  className,
  ...
  return (
    <div className={theme === 'dark' ? 'dark' : undefined}>
      <div
        data-grok
        className={className}
        style={{ background: 'var(--sm-bg-elevated)', color: 'var(--sm-text-primary)' }}
      >
        {children}
      </div>
    </div>
...
function GrokShowcase({ theme }: { theme: BenchTheme }) {
  // One representative identity per state (varied hue × shape), so a state is
  // never judged through a single silhouette across the size ladder.
  const models = CAST
  return (
    <div className="flex flex-col gap-6">
      {/* states × the size ladder — LIVE, so ambient motion is reviewable */}
      <GrokIsland theme={theme} className="overflow-x-auto rounded-2xl p-5">
        <table className="w-full min-w-[560px] border-collapse text-left">
          <thead>
            <tr>
              <th className="w-40 pb-3 text-[11px] font-medium opacity-70">state</th>
              {GROK_SIZES.map((s) => (
                <th key={s} className="pb-3 text-center text-[11px] font-medium tabular-nums opacity-70">
                  {s}px
                </th>
              ))}
              <th className="pb-3 pl-4 text-[11px] font-medium opacity-70">reads as</th>
            </tr>
          </thead>
          <tbody>
            {GROK_STATES.map((state, i) => {
              const m = models[i % models.length]
              return (
                <tr key={state} data-grok-state={state} className="border-t-[0.5px] border-white/10">
                  <th scope="row" className="py-3 pr-4 text-xs font-medium">
                    {state}
                  </th>
                  {GROK_SIZES.map((size) => (
                  ...
                  <td className="py-3 pl-4 text-[11px] opacity-60">{GROK_STATE_NOTE[state]}</td>
                </tr>
              )
            })}
          </tbody>
      ...
      {/* the attention layer — orthogonal to state, driven by the real fields */}
      <GrokIsland theme={theme} className="flex flex-wrap items-center gap-8 rounded-2xl p-5">
        <div className="flex flex-col items-center gap-2">
          <SessionMark seed="chat-dataplane" size={44} state="waiting" attention="needs" grok label={null} />
          <span className="text-[11px] opacity-70">attention: needs (pulsing)</span>
        </div>
        <div className="flex flex-col items-center gap-2">
          <SessionMark seed="remote-ssh" size={44} state="failed" attention="blocked" grok label={null} />
          <span className="text-[11px] opacity-70">attention: blocked (steady)</span>
        </div>
        <div className="flex flex-col items-center gap-2">
          <SessionMark seed="deploy-fix" size={44} state="streaming" grok animate label={null} />
          <span className="text-[11px] opacity-70">streaming (own-hue voice)</span>
        </div>
        <p className="max-w-xs text-[11px] leading-relaxed opacity-60">
          The colour firewall: busy glows in the agent&apos;s own hue, red is reserved for
          needs/blocked. With Reduce Motion on, every pulse freezes to its still and the halo,
          the glow and the pose all remain.
        </p>
      </GrokIsland>

      {/* identity: the 9 blob archetypes hold apart at idle, one hue per shape */}
      <GrokIsland theme={theme} className="flex flex-wrap items-end gap-x-6 gap-y-4 rounded-2xl p-5">
        {SILHOUETTES.map((silhouette, i) => (
          <div key={silhouette} className="flex w-16 flex-col items-center gap-1.5">
            <SessionMark
              seed={`${silhouette}-blob`}
              pin={{ silhouette, hue: HUE_SLOTS[i % HUE_SLOTS.length] }}
              size={40}
              grok
              animate
              label={null}
            />
            <span className="w-full truncate text-center text-[10px] leading-none opacity-60">
              {silhouette}
            </span>
          </div>
        ))}
  ...
  return (
    <div data-theme={theme} data-bench-theme={theme} className="bg-paper text-ink">
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-12 px-6 py-12">
        ...
        <Section
          id="grok"
          title="Grok skin — expressive faces (blob · negative eyes · status)"
          note="The [data-grok] upgrade. Body is an organic blob (adapted blobatar recipe, one static path, seeded per slug); eyes read as cut-outs of the page; and the status drives a small eye vocabulary + a hue-coherent glow/halo — no mouth. Nine states on the size ladder (LIVE, so ambient motion shows), the decoupled attention layer, and the nine blob archetypes at rest. Every loop drops to an informative still under Reduce Motion. This whole section is byte-inert off the skin — remove [data-grok] and the marks are the base faces above."
        >
          <GrokShowcase theme={theme} />
        </Section>
